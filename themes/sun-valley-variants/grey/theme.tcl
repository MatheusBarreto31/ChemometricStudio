...
namespace eval ttk::theme::sv_grey {
  package provide ttk::theme::sv_grey 1.0
  ...
  proc load_images {imgfile} {
    variable I
    image create photo sv_sheet_grey -file $imgfile -format png
    foreach {name x y width height} $::spriteinfo {
      set I($name) [image create photo -width $width -height $height]
      $I($name) copy sv_sheet_grey -from $x $y [expr {$x+$width}] [expr {$y+$height}]
    }
    image delete sv_sheet_grey
  }

  load_images "D:/cm-studio/themes/sun-valley-variants/grey/spritesheet.png"

  ttk::style theme create sv-grey -parent clam -settings {
    ...
    ttk::style map Treeview -background {selected "#0078d7"} -foreground {selected "#ffffff"}
