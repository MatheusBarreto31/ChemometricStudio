source "D:/cm-studio/venv/Lib/site-packages/sv_ttk/theme/sprites_light.tcl"

namespace eval ttk::theme::sv_light_blue {
  package provide ttk::theme::sv_light_blue 1.0
  ...
  proc load_images {imgfile} {
    variable I
    image create photo sv_sheet_light_blue -file $imgfile -format png
    foreach {name x y width height} $::spriteinfo {
      set I($name) [image create photo -width $width -height $height]
      $I($name) copy sv_sheet_light_blue -from $x $y [expr {$x+$width}] [expr {$y+$height}]
    }
    image delete sv_sheet_light_blue
  }

  load_images "D:/cm-studio/themes/sun-valley-variants/light-blue/spritesheet.png"

  ttk::style theme create sv-light-blue -parent clam -settings {
    ...
    ttk::style map Treeview -background {selected "#b7d0e4"} -foreground {selected "#10263d"}
