source "D:/cm-studio/venv/Lib/site-packages/sv_ttk/theme/sprites_dark.tcl"

namespace eval ttk::theme::sv_cobalt {
  package provide ttk::theme::sv_cobalt 1.0
  ...
  proc load_images {imgfile} {
    variable I
    image create photo sv_sheet_cobalt -file $imgfile -format png
    foreach {name x y width height} $::spriteinfo {
      set I($name) [image create photo -width $width -height $height]
      $I($name) copy sv_sheet_cobalt -from $x $y [expr {$x+$width}] [expr {$y+$height}]
    }
    image delete sv_sheet_cobalt
  }

  load_images "D:/cm-studio/themes/sun-valley-variants/cobalt/spritesheet.png"

  ttk::style theme create sv-cobalt -parent clam -settings {
    ...
    ttk::style map Treeview -background {selected "#1c3c42"} -foreground {selected "#dcdfe4"}
