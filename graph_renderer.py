...
def _normalize_force_integer_mode(value: Any) -> str:
    """Normalize force_integer config into one of: false, true, conditional."""
    if isinstance(value, str):
        normalized = value.strip().lower()
...
def _is_integer_like_numeric_values(values: Any, atol: float = 1e-9) -> bool:
    """Return True when all finite numeric values are integer-like."""
    try:
    ...
    return bool(np.all(np.isclose(finite, np.round(finite), atol=atol)))


def _collect_axis_numeric_values(ax, axis_name: str) -> np.ndarray:
    """Collect rendered numeric values for a single axis from plot artists."""
    ...
    # Standard lines (2D)
    for line in ax.get_lines() if hasattr(ax, 'get_lines') else []:
        if normalized_axis == 'x':
            _append(line.get_xdata(orig=False))
        elif normalized_axis == 'y':
            _append(line.get_ydata(orig=False))

    # Scatter collections (2D + 3D)
    for coll in getattr(ax, 'collections', []):
        if normalized_axis in {'x', 'y'} and hasattr(coll, 'get_offsets'):
            try:
                offsets = coll.get_offsets()
                if offsets is not None:
                    idx = 0 if normalized_axis == 'x' else 1
                    _append(np.asarray(offsets)[:, idx])
            except Exception:
                pass

        # 3D scatter stores coordinates in _offsets3d
        if hasattr(coll, '_offsets3d'):
            try:
                x_vals, y_vals, z_vals = coll._offsets3d
                if normalized_axis == 'x':
                    _append(x_vals)
                elif normalized_axis == 'y':
                    _append(y_vals)
                elif normalized_axis == 'z':
                    _append(z_vals)
            except Exception:
                pass

    # Fallback: infer from current tick locations when no artist values are available.
    if not collected:
        axis_obj = getattr(ax, f'{normalized_axis}axis', None)
        if axis_obj is not None and hasattr(axis_obj, 'get_ticklocs'):
            try:
                _append(axis_obj.get_ticklocs())
            except Exception:
                pass
...
def _should_force_integer_ticks(ax, axis_cfg: dict, axis_name: str) -> bool:
    """Resolve force_integer mode for one axis against rendered values."""
    mode = _normalize_force_integer_mode(axis_cfg.get('force_integer', False))
    if mode == 'true':
        return True
    if mode != 'conditional':
        return False

    values = _collect_axis_numeric_values(ax, axis_name)
...
def render_graph_figure(graph_type: str, config: dict, x_data: Optional[np.ndarray],
    ...
    # and we deliberately exclude that expansion from the equal-scale computation.
    _equal_scale_xlim: Optional[Tuple[float, float]] = None
    _equal_scale_ylim: Optional[Tuple[float, float]] = None
    if graph_type == 'scatter' and not use_3d and config.get('equal_scale', False):
        _equal_scale_xlim = ax.get_xlim()
        _equal_scale_ylim = ax.get_ylim()
    ...
    if _equal_scale_xlim is not None and _equal_scale_ylim is not None:
        _apply_equal_scale(ax, _equal_scale_xlim, _equal_scale_ylim)
...
def _apply_axis_tick_options(ax, config: dict, use_3d: bool = False) -> None:
    ...
    Supported per-axis options:
        - force_integer: bool or str in {'true','false','conditional'}
          (conditional enforces integer ticks only when axis values are integer-like)
    """
    x_axis_cfg = config.get('x_axis', {})
    y_axis_cfg = config.get('y_axis', {})
    z_axis_cfg = config.get('z_axis', {})

    if _should_force_integer_ticks(ax, x_axis_cfg, 'x'):
        ax.xaxis.set_major_locator(MaxNLocator(integer=True))

    if _should_force_integer_ticks(ax, y_axis_cfg, 'y'):
        ax.yaxis.set_major_locator(MaxNLocator(integer=True))

    if use_3d and hasattr(ax, 'zaxis') and _should_force_integer_ticks(ax, z_axis_cfg, 'z'):
...
def _apply_equal_scale(ax: Any, xlim: Tuple[float, float], ylim: Tuple[float, float]) -> None:
    """Force identical X and Y axis limits for a 2-D scatter plot.

    Computes a single range that spans all X *and* Y data (using limits
    captured before reference lines are drawn) and applies that same
    [lo, hi] to both axes.  This makes both axes show the same values so
    that, for example, a 1:1 diagonal reference line appears at exactly 45°.
    """
    ...
    # Single unified bounds that cover all X and Y values.
    all_min = min(x_min, y_min)
    all_max = max(x_max, y_max)
    total_range = all_max - all_min

    if total_range <= 0:
        return

    center = (all_min + all_max) / 2.0
    half = total_range / 2.0
...
def _finite_in_axis_transform(axis_obj: Any, values: np.ndarray) -> np.ndarray:
    """Return a finite-mask after applying the axis scale transform."""
    arr = np.asarray(values, dtype=float)
    try:
        transform = axis_obj.get_transform() if axis_obj is not None else None
        transformed = transform.transform(arr) if transform is not None else arr
        transformed_arr = np.asarray(transformed, dtype=float)
        if transformed_arr.shape != arr.shape:
            transformed_arr = np.reshape(transformed_arr, arr.shape)
        return np.isfinite(transformed_arr)
    except Exception:
        return np.isfinite(arr)


def _render_segment_preserving_linear_trajectory(
    ax: Any,
    x1: float,
    y1: float,
    x2: float,
    ...
    """Render a line segment in data-space while respecting non-linear axis transforms.

    When an axis is non-linear (e.g., log), one or both endpoints can be invalid
    (for example, 0 on log scale). Instead of dropping the segment, interpolate in
    linear data space and draw all transformed-valid portions.
    """
    n_points = 1024
    ...
    finite_x = _finite_in_axis_transform(getattr(ax, 'xaxis', None), x_vals)
    finite_y = _finite_in_axis_transform(getattr(ax, 'yaxis', None), y_vals)
    valid = finite_x & finite_y
    if not np.any(valid):
        return

    valid_idx = np.where(valid)[0]
    run_start = valid_idx[0]
    prev = valid_idx[0]

    def _draw_run(start: int, end: int) -> None:
        if end - start < 1:
            return
        ax.plot(
            x_vals[start:end + 1],
...
def _normalize_line_orientation(line_cfg: Dict[str, Any]) -> str:
    ...
    if token in {'h', 'horizontal', 'hline', 'axhline'}:
        return 'horizontal'
    if token in {'path', 'polyline', 'curve'}:
        return 'path'
    if token in {'segment', 'line', 'between', 'between_points', 'points'}:
        return 'segment'
    if token in {'custom_ellipsis', 'ellipsis', 'ellipse', 'ejcr'}:
        return 'custom_ellipsis'
    return ''


def _has_custom_ellipsis_entries(config: dict) -> bool:
    """Return True if the resolved config has any custom_ellipsis reference line entries."""
    ...
    if not isinstance(raw_lines, list):
        return False
    return any(
        isinstance(entry, dict) and _normalize_line_orientation(entry) == 'custom_ellipsis'
        for entry in raw_lines
    )


def _is_custom_ellipsis_visible(entry: Dict[str, Any], config: dict) -> bool:
    """Return False when this custom_ellipsis entry is hidden via the confidence level toggle.

    An entry is hidden when its ``confidence_level`` value appears in the
    ``hidden_confidence_levels`` list stored in the graph config.  Entries
    without a ``confidence_level`` key are always visible.
    """
    cl = entry.get('confidence_level')
    if cl is None:
        return True
    cl_str = str(cl).strip().replace('%', '')
    hidden = [str(h).strip().replace('%', '') for h in config.get('hidden_confidence_levels', [])]
    return cl_str not in hidden
...
def _render_scatter_reference_lines(ax, config: dict) -> None:
    ...
    for line_cfg in raw_lines:
        ...
        if orientation == 'vertical':
        ...
        elif orientation == 'path':
            x_raw = line_cfg.get('x', line_cfg.get('x_values'))
            y_raw = line_cfg.get('y', line_cfg.get('y_values'))

            points = line_cfg.get('points')
            if points is not None and (x_raw is None or y_raw is None):
                if isinstance(points, (list, tuple)):
                    x_raw = [p[0] for p in points if isinstance(p, (list, tuple)) and len(p) >= 2]
                    y_raw = [p[1] for p in points if isinstance(p, (list, tuple)) and len(p) >= 2]

            x_vals_raw = _as_line_list(x_raw)
            y_vals_raw = _as_line_list(y_raw)
            n_points = min(len(x_vals_raw), len(y_vals_raw))
            if n_points <= 1:
                continue
            ...
            for i in range(n_points):
                x_val = _coerce_float(x_vals_raw[i])
                y_val = _coerce_float(y_vals_raw[i])
                x_vals.append(np.nan if x_val is None else float(x_val))
                y_vals.append(np.nan if y_val is None else float(y_val))

            x_arr = np.asarray(x_vals, dtype=float)
            y_arr = np.asarray(y_vals, dtype=float)

            # Determine intended axis scale from config rather than the live axis state,
            # because _apply_axis_scale_options may not have been called yet at this point
            # (it is deliberately applied after all drawing to prevent autoscaling resets).
            x_is_log = _normalize_axis_type(config.get('x_axis', {})) in ('log10', 'log2', 'ln')
            y_is_log = _normalize_axis_type(config.get('y_axis', {})) in ('log10', 'log2', 'ln')

            valid = np.isfinite(x_arr) & np.isfinite(y_arr)
            if x_is_log:
                valid = valid & (x_arr > 0.0)
            if y_is_log:
                valid = valid & (y_arr > 0.0)
            valid_idx = np.flatnonzero(valid)
            if valid_idx.size == 0:
                continue

            color = _pick_indexed(style_color, 0, 'gray')
            linestyle = _pick_indexed(style_ls, 0, '--')
            linewidth = _coerce_float(_pick_indexed(style_lw, 0, 1.0))
            alpha = _coerce_float(_pick_indexed(style_alpha, 0, 0.9))
            label = _pick_indexed(labels_cfg, 0)
            expand_limits = _as_bool(line_cfg.get('expand_limits', False))

            run_start = int(valid_idx[0])
            prev = int(valid_idx[0])

            def _draw_run(start_idx: int, end_idx: int) -> None:
                if end_idx - start_idx + 1 < 2:
                    return
                ax.plot(
                    x_arr[start_idx:end_idx + 1],
                    y_arr[start_idx:end_idx + 1],
                    color=color,
                    linestyle=linestyle,
                    linewidth=linewidth if linewidth is not None else 1.0,
                    alpha=alpha if alpha is not None else 0.9,
            ...
            # Snapshot the data-limit bounding box that has been accumulated from
            # the main scatter/line data before drawing the decision-boundary path.
            # The path may extend far beyond the data range (to cover the full
            # visible region in log space) but must not push the autoscaled limits
            # outward.  After drawing we restore the snapshot so that the later
            # _apply_axis_scale_options call autoscales from the data alone.
            _dataLim_snapshot = ax.dataLim.frozen()
            _snapshot_pts = _dataLim_snapshot.get_points()
            _has_real_data = not np.any(np.isinf(_snapshot_pts))
            ...
            # Restore data limits so the path does not expand the visible region.
            if _has_real_data and not expand_limits:
                ax.dataLim.set_points(_snapshot_pts)

            if label is not None and str(label).strip() != '':
                label_position = str(line_cfg.get('label_position', 'middle')).strip().lower()
                label_side = str(line_cfg.get('label_side', 'above')).strip().lower()
                label_offset_points = _coerce_float(line_cfg.get('label_offset_points', 6.0))
                if label_offset_points is None:
                    label_offset_points = 6.0

                if _has_real_data:
                    x_min, y_min = float(_snapshot_pts[0, 0]), float(_snapshot_pts[0, 1])
                    x_max, y_max = float(_snapshot_pts[1, 0]), float(_snapshot_pts[1, 1])
                else:
                    x_min, x_max = ax.get_xlim()
                    y_min, y_max = ax.get_ylim()
                x_lo, x_hi = (x_min, x_max) if x_min <= x_max else (x_max, x_min)
                y_lo, y_hi = (y_min, y_max) if y_min <= y_max else (y_max, y_min)
                visible = valid & (x_arr >= x_lo) & (x_arr <= x_hi) & (y_arr >= y_lo) & (y_arr <= y_hi)
                visible_idx = np.flatnonzero(visible)
                anchor_idx_arr = visible_idx if visible_idx.size > 0 else valid_idx
                ...
                frac = frac_map.get(label_position, 0.5)
                rel_idx = int(round((anchor_idx_arr.size - 1) * frac)) if anchor_idx_arr.size > 1 else 0
                rel_idx = max(0, min(rel_idx, anchor_idx_arr.size - 1))
                label_idx = int(anchor_idx_arr[rel_idx])

                if label_side == 'below':
                    xytext = (0.0, -label_offset_points)
                    ha, va = 'center', 'top'
                elif label_side == 'left':
                    xytext = (-label_offset_points, 0.0)
                    ha, va = 'right', 'center'
                elif label_side == 'right':
                    xytext = (label_offset_points, 0.0)
                    ha, va = 'left', 'center'
                elif label_side == 'center':
                    xytext = (0.0, 0.0)
                    ha, va = 'center', 'center'
                else:
                    xytext = (0.0, label_offset_points)
                ...
                ax.annotate(
                    str(label),
                    xy=(x_arr[label_idx], y_arr[label_idx]),
                    xytext=xytext,
        ...
        elif orientation == 'custom_ellipsis':
            if not _is_custom_ellipsis_visible(line_cfg, config):
                continue

            color = _pick_indexed(style_color, 0, 'steelblue')
            linestyle = _pick_indexed(style_ls, 0, '-')
            linewidth = _coerce_float(_pick_indexed(style_lw, 0, 1.5))
            alpha = _coerce_float(_pick_indexed(style_alpha, 0, 0.9))
            label = _pick_indexed(labels_cfg, 0)
            marker = line_cfg.get('marker')
            markersize = _coerce_float(line_cfg.get('markersize', 7))
            expand_limits = _as_bool(line_cfg.get('expand_limits', False))

            x_raw = line_cfg.get('x', line_cfg.get('x_values'))
            y_raw = line_cfg.get('y', line_cfg.get('y_values'))
            packed = line_cfg.get('ellipses_data', line_cfg.get('packed_ellipses'))

            if isinstance(packed, dict):
                x_paths = packed.get('x_paths')
                y_paths = packed.get('y_paths')
                if x_paths is not None and y_paths is not None:
                    try:
                        x_paths_arr = np.asarray(x_paths, dtype=float)
                        y_paths_arr = np.asarray(y_paths, dtype=float)
                    except Exception:
                        x_paths_arr = None
                        y_paths_arr = None

                    if (
                        isinstance(x_paths_arr, np.ndarray)
                        and isinstance(y_paths_arr, np.ndarray)
                        and x_paths_arr.ndim >= 2
                        and y_paths_arr.ndim >= 2
                    ):
                        packed_labels = _as_line_list(packed.get('labels', []))
                        packed_conf_levels = _as_line_list(packed.get('confidence_levels', []))
                        packed_linestyles = _as_line_list(packed.get('linestyles', []))
                        packed_linewidths = _as_line_list(packed.get('linewidths', []))
                        packed_alphas = _as_line_list(packed.get('alphas', []))
                        packed_markers = _as_line_list(packed.get('markers', []))
                        packed_markersizes = _as_line_list(packed.get('markersizes', []))
                        packed_colors = _as_line_list(packed.get('colors', []))
                        packed_expand = _as_bool(packed.get('expand_limits', expand_limits))

                        n_items = min(x_paths_arr.shape[0], y_paths_arr.shape[0])
                        for i in range(n_items):
                            x_arr = np.asarray(x_paths_arr[i], dtype=float).reshape(-1)
                            y_arr = np.asarray(y_paths_arr[i], dtype=float).reshape(-1)
                            if x_arr.size == 0 or y_arr.size == 0:
                                continue
                            n_pts = min(x_arr.size, y_arr.size)
                            x_arr = x_arr[:n_pts]
                            y_arr = y_arr[:n_pts]

                            level = _pick_indexed(packed_conf_levels, i)
                            visibility_entry = {'confidence_level': level}
                            if not _is_custom_ellipsis_visible(visibility_entry, config):
                                continue

                            p_color = _pick_indexed(packed_colors, i, color)
                            p_ls = _pick_indexed(packed_linestyles, i, linestyle)
                            p_lw = _coerce_float(_pick_indexed(packed_linewidths, i, linewidth))
                            p_alpha = _coerce_float(_pick_indexed(packed_alphas, i, alpha))
                            p_label = _pick_indexed(packed_labels, i)
                            p_marker = _pick_indexed(packed_markers, i, marker)
                            p_markersize = _coerce_float(_pick_indexed(packed_markersizes, i, markersize))
                            ...
                            effective_ls = str(p_ls).lower() if p_ls is not None else '-'
                            effective_lw = p_lw if p_lw is not None else 1.5

                            if finite_idx.size == 1 and p_marker:
                                ax.plot(
                                    x_arr[valid],
                                    y_arr[valid],
                                    linestyle='none',
                                    marker=p_marker,
                                    markersize=p_markersize if p_markersize is not None else 7,
                                    color=p_color,
                                    alpha=p_alpha if p_alpha is not None else 0.9,
                                    zorder=6,
                                )
                            else:
                                if (
                                    finite_idx.size > 1
                                    and not (
                                        np.isclose(x_arr[finite_idx[0]], x_arr[finite_idx[-1]])
                                        and np.isclose(y_arr[finite_idx[0]], y_arr[finite_idx[-1]])
                                    )
                                ):
                                    x_arr = np.append(x_arr, x_arr[finite_idx[0]])
                                    y_arr = np.append(y_arr, y_arr[finite_idx[0]])
                                ...
                                if effective_ls not in ('none', ''):
                                    plot_kwargs['linestyle'] = effective_ls
                                    plot_kwargs['linewidth'] = effective_lw
                                if p_marker:
                                    plot_kwargs['marker'] = p_marker
                                    plot_kwargs['markersize'] = p_markersize if p_markersize is not None else 7

                                ax.plot(x_arr, y_arr, **plot_kwargs)

                            if _has_real_data and not packed_expand:
                                ax.dataLim.set_points(_snapshot_pts)

                            if p_label is not None and str(p_label).strip() != '':
                                lbl_idx = int(finite_idx[finite_idx.size // 2])
                                ax.text(
                                    x_arr[lbl_idx],
                                    y_arr[lbl_idx],
                                    str(p_label),
            ...
            if x_raw is not None and y_raw is not None:
                ...
                x_vals_raw = _as_line_list(x_raw)
                y_vals_raw = _as_line_list(y_raw)
                n_pts = min(len(x_vals_raw), len(y_vals_raw))
                if n_pts < 1:
                    continue

                x_arr = np.asarray(
                    [
                        _coerce_float(v) if _coerce_float(v) is not None else np.nan
                        for v in x_vals_raw[:n_pts]
                    ],
                    dtype=float,
                )
                y_arr = np.asarray(
                ...
                effective_ls = str(linestyle).lower() if linestyle is not None else '-'
                effective_lw = linewidth if linewidth is not None else 1.5

                finite_idx = np.flatnonzero(valid)
                if finite_idx.size == 1 and marker:
                    # Single-point marker (e.g. ideal-point indicator)
                    ax.plot(
                        x_arr[valid],
                        y_arr[valid],
                        linestyle='none',
                        marker=marker,
                        markersize=markersize if markersize is not None else 7,
                        color=color,
                        alpha=alpha if alpha is not None else 0.9,
                        zorder=6,
                    )
                else:
                    # Multi-point path (ellipse boundary) — close it if not already closed
                    if (
                        finite_idx.size > 1
                        and not (
                            np.isclose(x_arr[finite_idx[0]], x_arr[finite_idx[-1]])
                    ...
                    if marker:
                        ...
                        plot_kwargs['markersize'] = markersize if markersize is not None else 7
                ...
                # --- Parametric format: center + width + height + angle ---
                center_raw = line_cfg.get('center', line_cfg.get('xy'))
                if center_raw is None:
                    continue
                center_list = _as_line_list(center_raw)
                if len(center_list) < 2:
                    continue
                cx = _coerce_float(center_list[0])
                cy = _coerce_float(center_list[1])
                if cx is None or cy is None or not (np.isfinite(cx) and np.isfinite(cy)):
                    continue

                width_raw = line_cfg.get('width')
                if width_raw is None:
                    semi_x = _coerce_float(line_cfg.get('semi_x'))
                    width_raw = semi_x * 2.0 if semi_x is not None else None

                height_raw = line_cfg.get('height')
                if height_raw is None:
                    semi_y = _coerce_float(line_cfg.get('semi_y'))
                    height_raw = semi_y * 2.0 if semi_y is not None else None

                width = _coerce_float(width_raw)
                height = _coerce_float(height_raw)
                angle = _coerce_float(line_cfg.get('angle', 0.0)) or 0.0
            ...
            for i in range(n_lines):
                ...
                _render_segment_preserving_linear_trajectory(
                    ax=ax,
                    x1=x1,
                    y1=y1,
                    x2=x2,
...
def _render_scatter(ax, x_data: Optional[np.ndarray], y_data: Optional[np.ndarray],
    ...
    # Otherwise use traditional single dataset rendering
        ...
        if not use_3d and _is_confidence_ellipses_enabled(config) and not _has_custom_ellipsis_entries(config):
            default_color = 'C0'
...
def _render_scatter_multi_dataset(ax, datasets: List[Dict[str, Any]], config: dict, 
    ...
    def _discrete_color_map(values: np.ndarray, cmap_name: str, preferred_order: Optional[List[str]] = None) -> Tuple[np.ndarray, Dict[str, Any]]:
        ...
        if preferred_order:
            preferred_lookup = [str(v) for v in preferred_order]
            leftovers = [v for v in unique_vals if str(v) not in preferred_lookup]
            mapping: Dict[str, Any] = {}
            for v in unique_vals:
                value_str = str(v)
                if value_str in preferred_lookup:
                    palette_idx = preferred_lookup.index(value_str)
                else:
                    palette_idx = len(preferred_lookup) + leftovers.index(v)
                mapping[value_str] = mcolors.to_rgba(palette[palette_idx % len(palette)])
        else:
    ...
    class_value_order_cfg = config.get('class_value_order_effective', config.get('class_value_order'))
    if isinstance(class_value_order_cfg, (list, tuple)):
        class_value_order = [str(v) for v in class_value_order_cfg]
    else:
        class_value_order = None
        ...
        if class_layers is None or class_layers.size == 0:
            ...
            if 'color' in aspect_layer_values:
                ...
                if use_cont:
                    ...
                    face_rgba, discrete_map = _discrete_color_map(color_values, color_cmap_qual, class_value_order)
            ...
            if 'edge' in aspect_layer_values:
                ...
                if use_cont_edge:
                    ...
                    edge_rgba, edge_discrete_map = _discrete_color_map(edge_values, edge_cmap_qual, class_value_order)
    ...
    if not use_3d and _is_confidence_ellipses_enabled(config) and not _has_custom_ellipsis_entries(config) and len(datasets) > 0:
        confidence = _parse_confidence_level(config)
...
def _render_line(ax, x_data: Optional[np.ndarray], y_data: Optional[np.ndarray],
    ...
    # Otherwise use traditional single dataset rendering
        ...
        if isinstance(y_data, np.ndarray) and y_data.ndim == 2:
            ...
            # Match multi-dataset fallback behavior: when x/y lengths are incompatible,
...
def _render_line_multi_dataset(ax, datasets: List[Dict[str, Any]], config: dict,
    ...
    def _prepare_line_rows(x_values: Any, y_values: Any) -> Tuple[List[Optional[np.ndarray]], List[np.ndarray]]:
        ...
        x_rows: List[Optional[np.ndarray]] = [None] * n_rows

        if x_values is not None:
            try:
                x_arr = np.asarray(x_values)
                if x_arr.ndim == 1 and int(x_arr.shape[0]) == int(n_cols):
                    x_vec = np.asarray(x_arr, dtype=float)
                    x_rows = [x_vec] * n_rows
                elif x_arr.ndim == 2 and int(x_arr.shape[0]) == int(n_rows) and int(x_arr.shape[1]) == int(n_cols):
                    x_rows = [np.asarray(x_arr[i], dtype=float) for i in range(n_rows)]
            except Exception:
                # Fall back to implicit matplotlib indexing when x-values are unusable.
                x_rows = [None] * n_rows
...
def embed_figure_in_tkinter(
    fig: Figure,
    parent_frame: ttk.Frame,
    pinned_labels: Optional[set] = None,
    on_label_pin_toggled: Optional[Callable[[str, bool], None]] = None,
) -> Tuple[FigureCanvasTkAgg, ttk.Frame]:
    ...
    _setup_scatter_tooltips(
        canvas,
        fig,
        pinned_labels=pinned_labels,
        on_label_pin_toggled=on_label_pin_toggled,
    )
    
    return canvas, canvas_frame


def _setup_scatter_tooltips(
    ...
    active_pinned_labels = {
        str(label).strip()
        for label in (pinned_labels or set())
        if str(label).strip()
    }
    callback_ref = {'fn': on_label_pin_toggled}

    def _normalize_label(label: Any) -> Optional[str]:
        text = str(label).strip() if label is not None else ''
        return text if text else None

    def _resolve_point_label(scatter, ind: int) -> Optional[str]:
        sample_labels = getattr(scatter, 'sample_labels', None)
        if sample_labels is None:
            return None
        try:
            raw = sample_labels[ind]
        except Exception:
            return None
    ...
    def _iter_matching_points(label_text: str):
        for scatter in scatter_collections:
            ...
            n_points = min(
                len(sample_labels),
                len(getattr(scatter, 'x_data', [])),
                len(getattr(scatter, 'y_data', [])),
            )
            for idx in range(n_points):
                current = _resolve_point_label(scatter, idx)
                if current == label_text:
                    yield scatter, idx

    def _unpin_label(label_text: str) -> bool:
        removed_any = False
        for ann in tooltip_state['pinned_annotations'].pop(label_text, []):
            try:
                ann.remove()
                removed_any = True
            except Exception:
                pass
        active_pinned_labels.discard(label_text)
        return removed_any

    def _pin_label(label_text: str) -> bool:
        _unpin_label(label_text)
        created = []
        for scatter, idx in _iter_matching_points(label_text):
            ann = _make_annotation(scatter, idx, is_pinned=True)
            if ann is not None:
                created.append(ann)
        if not created:
            active_pinned_labels.discard(label_text)
            return False
        tooltip_state['pinned_annotations'][label_text] = created
        active_pinned_labels.add(label_text)
        return True

    def _notify_pin_toggle(label_text: str, should_pin: bool) -> None:
        callback_fn = callback_ref.get('fn')
        if callback_fn is None:
            return
        try:
            callback_fn(label_text, bool(should_pin))
        except Exception:
            pass

    def _apply_external_pins(labels: Optional[set]) -> None:
        desired = {
            str(label).strip()
        ...
        for existing in list(active_pinned_labels):
            if existing not in desired:
                changed = _unpin_label(existing) or changed

        for target in sorted(desired):
            if target not in active_pinned_labels:
                changed = _pin_label(target) or changed

        if tooltip_state['hover_annotation'] is not None and tooltip_state['hover_key'] is not None:
            scatter_id, point_index = tooltip_state['hover_key']
            hovered_label = None
            for scatter in scatter_collections:
                if id(scatter) == scatter_id:
                    hovered_label = _resolve_point_label(scatter, point_index)
                    break
            if hovered_label and hovered_label in active_pinned_labels:
                try:
    ...
    def on_motion(event):
        ...
        point_label = _resolve_point_label(scatter, point_index)

        # If the label is pinned, do not draw a duplicate hover annotation.
        if point_label and point_label in active_pinned_labels:
    ...
    def on_click(event):
        ...
        if point_label in active_pinned_labels:
            _unpin_label(point_label)
            _notify_pin_toggle(point_label, False)
        else:
            created = _pin_label(point_label)
            if created:
                _notify_pin_toggle(point_label, True)
        canvas.draw_idle()

    for label in sorted(active_pinned_labels):
        _pin_label(label)

    try:
        setattr(canvas, '_scatter_tooltip_set_pins', _apply_external_pins)
        setattr(canvas, '_scatter_tooltip_set_callback', lambda fn: callback_ref.__setitem__('fn', fn))
    except Exception:
        pass
...
def update_embedded_figure(fig: Figure, instance_alias: str, section_id: Union[int, Tuple[int, int]],
                          analysis_data: dict, canvas_frame: ttk.Frame,
                          pinned_labels: Optional[set] = None,
                          on_label_pin_toggled: Optional[Callable[[str, bool], None]] = None) -> None:
    ...
    if 'graph_canvases' in analysis_data.get(instance_alias, {}):
        ...
        if canvas_data:
            ...
            if stored_canvas_frame is None:
                return
            try:
                if not stored_canvas_frame.winfo_exists():
                    return
            except tk.TclError:
                return
            ...
            try:
                ...
                if not old_widget.winfo_exists():
                    raise tk.TclError("Old canvas widget no longer exists")
            except Exception:
                # Widget was already destroyed/replaced by another refresh.
                # Re-embed the new figure in the stored frame and refresh reference.
...
def sync_canvas_pinned_labels(
    canvas: FigureCanvasTkAgg,
    pinned_labels: Optional[set],
    on_label_pin_toggled: Optional[Callable[[str, bool], None]] = None,
) -> None:
    """Synchronize pinned scatter labels on an existing embedded canvas in-place."""
    if canvas is None:
        return
    ...
    set_callback = getattr(canvas, '_scatter_tooltip_set_callback', None)
    if callable(set_callback):
        try:
            set_callback(on_label_pin_toggled)
        except Exception:
            pass

    set_pins = getattr(canvas, '_scatter_tooltip_set_pins', None)
    if callable(set_pins):
        try:
            set_pins(pinned_labels)
        except Exception:
            pass
