...
def _resolve_padding_value(s_padding_type: Any) -> float:
    """Resolve padding strategy token into a numeric fill value."""
    token = str(s_padding_type).strip().lower()
    if token in {"zeroes", "zeros", "zero", "0"}:
        return 0.0
    return float("nan")


def _pad_samples_to_common_shape(samples: List[np.ndarray], pad_value: float) -> Tuple[np.ndarray, Optional[np.ndarray]]:
    """Pad samples to a common shape.

    Returns:
        padded_tensor: stacked tensor in sample-first layout.
        real_data_mask: boolean mask when padding is required, otherwise None.
    """
    if not samples:
        return np.empty((0,), dtype=float), None

    numeric_samples = [np.asarray(sample, dtype=float) for sample in samples]
    sample_ndim = int(numeric_samples[0].ndim)
    if any(int(sample.ndim) != sample_ndim for sample in numeric_samples):
        raise ValueError("All multiway samples must have the same number of dimensions")

    sample_shapes = [tuple(sample.shape) for sample in numeric_samples]
    reference_shape = sample_shapes[0]
    if all(shape == reference_shape for shape in sample_shapes):
        # Homogeneous shapes: direct stack with no mask to avoid unnecessary memory use.
        return np.stack(numeric_samples, axis=0), None

    max_shape = tuple(max(sample.shape[axis] for sample in numeric_samples) for axis in range(sample_ndim))
    tensor_shape = (len(numeric_samples),) + max_shape

    padded_tensor = np.full(tensor_shape, pad_value, dtype=float)
    real_data_mask = np.zeros(tensor_shape, dtype=bool)

    for sample_idx, sample in enumerate(numeric_samples):
        sample_slices = tuple(slice(0, dim) for dim in sample.shape)
        target_index = (sample_idx,) + sample_slices
        padded_tensor[target_index] = sample
        real_data_mask[target_index] = True

    return padded_tensor, real_data_mask
...
def load_data(d_specs_separator: str = "Auto detect", d_specs_headlines: str = "", d_specs_headcolumns: str = "", d_specs_type: str = "Auto detect", d_specs_dimensions: Optional[List[str]] = None,
              ...
              source_metadata_overrides: Optional[Dict[str, Dict[str, Any]]] = None,
              translation_keys: Optional[Dict[str, Any]] = None) -> Tuple[np.ndarray, Optional[np.ndarray], Optional[List[List[str]]], List[str], Optional[List[np.ndarray]], List[str], List[str], Optional[List[Any]], List[str], Dict[str, Dict[str, Any]], Optional[np.ndarray], Optional[np.ndarray]]:
    """
    ...
    Args:
        ...
        s_padding_type: Padding strategy for nway_flag >= 2 when sample shapes differ ('NaN' or 'Zeroes')
    ...
    Returns:
        ...
        cal_s_mask: Boolean mask for X_cal indicating real (non-padding) positions; None for nway_flag == 1
    """
    ...
    pad_value = _resolve_padding_value(s_padding_type)
    ...
    if multi_file_per_sample and nway_flag >= 3 and data_path and num_samples:
        ...
        X, cal_s_mask = _load_x_multifile_per_sample(sample_paths, separator, num_headlines, num_headcolumns, data_type,
                                 dimensions, nway_flag, transpose, reshape_order, pad_value)
        row_counts = []
        ...
        X, row_counts, cal_s_mask = _load_x_data(
    ...
    if Y is not None:
        ...
        padded: List[str] = []
        for _i in range(y_cols):
            if _i < len(resolved_y_labels) and str(resolved_y_labels[_i]).strip():
                padded.append(str(resolved_y_labels[_i]).strip())
            else:
                padded.append(f"Y{_i + 1}")
        resolved_y_labels = padded
    ...
    axis_t_info = _load_axis_text_info(
        var_path_list,
        smp_labels,
        nway_flag,
        axis_n_info,
    ...
    processed_dim_labels = _generate_dim_labels(dim_labels, nway_flag, translation_keys=translation_keys)
    ...
    return X, Y, axis_t_info, smp_labels, axis_n_info, axis_nature_list, processed_dim_labels, class_data, resolved_y_labels, cal_metadata, correlation_matrix, cal_s_mask
...
def _load_x_data(data_path: List[str], separator: Optional[str], num_headlines: int, num_header_columns: int,
                 data_type: str, dimensions: Optional[str], transpose: bool, nway_flag: int,
                 reshape_order: str = 'F', pad_value: float = float('nan')) -> Tuple[np.ndarray, List[int], Optional[np.ndarray]]:
    """Load and organize X data based on nway_flag and data_type."""
    # global nway_flag

    if nway_flag == 1:
        X, row_counts = _load_x_1way(data_path, separator, num_headlines, num_header_columns, data_type, transpose)
        return X, row_counts, None
    else:
        X, s_mask = _load_x_multiway(data_path, separator, num_headlines, num_header_columns, data_type, dimensions,
                                     nway_flag, transpose, reshape_order, pad_value)
        return X, [], s_mask  # No row_counts for multiway
...
def _load_x_multiway(data_path: List[str], separator: Optional[str], num_headlines: int, num_header_columns: int,
                     data_type: str, dimensions: Optional[str], nway_flag: int, transpose: Optional[bool],
                     reshape_order: str = 'F', pad_value: float = float('nan')) -> Tuple[np.ndarray, Optional[np.ndarray]]:
    ...
    # Stack into tensor with sample dimension first, padding heterogeneous sample shapes.
    return _pad_samples_to_common_shape(samples, pad_value)
...
def _load_x_multifile_per_sample(sample_paths: List[List[str]], separator: Optional[str], num_headlines: int, num_header_columns: int,
                                  ...
                                  transpose: Optional[bool], reshape_order: str = 'F',
                                  pad_value: float = float('nan')) -> Tuple[np.ndarray, Optional[np.ndarray]]:
...
def _load_axis_text_info(
    var_path: Optional[List[str]],
    smp_labels: List[str],
    nway_flag: int,
    axis_n_info: Optional[List[np.ndarray]],
    prefer_axis_numeric_labels: bool = False,
) -> List[List[str]]:
    """
    ...
    Args:
        ...
        prefer_axis_numeric_labels: If True, missing text labels are generated from
                       axis_n_info values when available.
    ...
    for i in range(1, len(axis_t_info)):
        if not axis_t_info[i]:  # Empty list
            ...
            if axis_n_info is not None and i < len(axis_n_info) and axis_n_info[i] is not None:
                axis_vector = np.asarray(axis_n_info[i]).reshape(-1)
                dim_size = len(axis_vector)

            if prefer_axis_numeric_labels and axis_vector is not None and dim_size > 0:
                # Mirror generated numeric axis vectors into text labels.
                axis_t_info[i] = [f"{float(v):g}" for v in axis_vector]
                continue
...
def _resolve_translation_term(translation_keys: Optional[Dict[str, Any]], key: str, default: str) -> str:
    """Resolve a localized term from function config translation keys."""
...
def _generate_dim_labels(dim_labels, nway_flag: int, translation_keys: Optional[Dict[str, Any]] = None) -> List[str]:
    """
    ...
    Args:
        ...
        nway_flag: Number of dimensions (excluding samples)
        translation_keys: Optional dict of localized term overrides from the function config
        
    Returns:
        List of dimension labels starting with "Samples" followed by user-provided names
        or default W-prefixed names for empty values (e.g., W1, W2); for first-order data
        (nway_flag == 1) the single non-sample direction defaults to "Variable".
    """
    ...
    # Resolve translatable default terms
    default_variable_label = _resolve_translation_term(translation_keys, "variable", "Variable")
    default_way_prefix = _resolve_translation_term(translation_keys, "way_prefix", "W")
    ...
    for i in range(nway_flag):
        if i < len(user_labels) and user_labels[i]:
            ...
            # First-order data: single non-sample direction uses "Variable"
            result.append(default_variable_label)
        else:
            # Multiway data: use W-prefixed default name (1-based)
            result.append(f"{default_way_prefix}{i + 1}")
