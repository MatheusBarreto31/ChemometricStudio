...
class _RandomFrog(SpectralSelector, BaseEstimator, metaclass=ABCMeta):
    ...
    def _select(self, X, y,
        ...
        for i in range(n_iterations):
            ...
            n_candidate_features = int(np.clip(round(n_candidate_features), n_components, n_features))
            ...
            if n_candidate_features < n_selected_features:
                ...
                # Expansion step
                n_diff = int(n_candidate_features - n_selected_features)
                non_selected_features = np.setdiff1d(all_features, selected_features)
                n_features_to_explore = int(
                    min(max(0, round(subset_expansion_factor * n_diff)), len(non_selected_features))
                )
