...
class IPLS(IntervalSelector):
    ...
    def __init__(self,
        ...
        if n_intervals_to_select != 1:
            warnings.warn("""IPLS only supports the selection of a single interval.
                             ...
                             n_intervals_to_select * interval_width. Hence, IPLS models the special case of arranging
                             the selected intervals as a continuum.""")
    ...
    def _fit(self, X, y, n_intervals_to_select, interval_width):
        candidates = Parallel(n_jobs=self.n_jobs)(delayed(self._fit_ipls)(
            X,
            y,
            interval_width * n_intervals_to_select,
            self.pls,
            i,
        ) for i in range(X.shape[-1] - interval_width + 1))
