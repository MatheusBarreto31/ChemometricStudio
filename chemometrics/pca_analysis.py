...
def _compute_pca_metrics(
    ...
    Args:
        ...
        loadings: PCA loadings (n_variables x n_components)
        mean_vector: Optional mean subtracted before PCA (for inverse transform)
        eigenvalues: Optional calibration eigenvalues (explained variance per component).
                     When provided (e.g. for validation data), T2 Hotelling uses these
    ...
    # Eigenvalues used for T2: prefer externally supplied calibration eigenvalues so that
    # validation/CV T2 values are comparable to calibration (same scale per component).
    if eigenvalues is not None:
        ev_for_t2 = np.asarray(eigenvalues, dtype=float)[:n_components]
    else:
        ev_for_t2 = explained_variance

    # Leverage: h_i = diag(T (T'T)^{-1} T') — per-sample influence in score space
    if n_samples > 1 and n_components > 0:
        try:
            TtT = scores.T @ scores  # (n_components x n_components)
            TtT_inv = np.linalg.pinv(TtT)
            # diag(T M T') = row-wise dot product of (T M) and T
            leverage = np.sum((scores @ TtT_inv) * scores, axis=1)
        except Exception:
            leverage = np.full(n_samples, np.nan)
    else:
        leverage = np.zeros(n_samples, dtype=float)

    # T2 Hotelling: T2_i = sum_k (t_ik^2 / lambda_k) — Mahalanobis distance in score space
    safe_ev = np.where(ev_for_t2 > 0, ev_for_t2, np.nan)
    t2_hotelling = np.nansum(scores ** 2 / safe_ev, axis=1)

    # Q-residuals (SPE): per-sample sum of squared reconstruction errors
    X_hat = scores @ loadings.T
    if mean_vector is not None:
        X_hat = X_hat + mean_vector
    q_residuals = np.sum((X_data - X_hat) ** 2, axis=1)
    ...
    return {
        ...
        't2_hotelling': t2_hotelling.tolist(),
        'q_residuals': q_residuals.tolist(),
    }
...
def _format_pct_value(value: Any) -> str:
    """Format a percentage value for display (drop trailing .0)."""
    try:
        numeric = float(value)
    except Exception:
        return "0"
...
def _pca_analysis_single_fit(
    ...
    **kwargs
    ...
    if translation_keys is None and 'translation_keys' in kwargs:
        translation_keys = kwargs['translation_keys']
