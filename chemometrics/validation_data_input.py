...
def validation_data_main(X_cal: np.ndarray, Y_cal: Optional[np.ndarray], smp_cal: List[str],
                          ...
                          s_padding_type: Optional[str] = None,
                          ...
                          X_val_path: Optional[Union[str, List[str]]] = None, Y_val_path: Optional[str] = None,
                          val_labels_path: Optional[str] = None,
                          cdata_path: Optional[str] = None,
                          source_metadata_overrides: Optional[dict] = None) -> Tuple[np.ndarray, Optional[np.ndarray], np.ndarray, Optional[np.ndarray], List[str], List[str], Optional[List[Any]], Optional[List[Any]], Optional[dict], Optional[dict], Optional[np.ndarray], Optional[np.ndarray]]:
    """
    ...
    Args:
        ...
        s_padding_type: Padding strategy inherited from load_data ('NaN' or 'Zeroes')
        cdata_path: Path to external classification data file

    Returns:
        X_cal, Y_cal, X_val, Y_val, smp_cal, smp_val, class_data_cal, class_data_val, cal_metadata, val_metadata, cal_s_mask, val_s_mask
    """
    ...
    if not should_create:
        ...
        if creationMethod == 'random':
        ...
        elif creationMethod == 'stratified_random':
            if class_data_cal is None:
                raise ValueError("stratified_random requires class_data_cal to be provided")
            ...
            sss = StratifiedShuffleSplit(n_splits=1, train_size=n_cal, random_state=random_seed)
        ...
        cal_s_mask_out = cal_s_mask[cal_indices] if cal_s_mask is not None else None
        ...
        val_s_mask = cal_s_mask[val_indices] if cal_s_mask is not None else None

    return X_cal_out, Y_cal_out, X_val, Y_val, smp_cal_out, smp_val, class_data_cal_out, class_data_val, cal_metadata_out, val_metadata, cal_s_mask_out, val_s_mask
...
def _normalize_path_list(paths: Optional[Union[str, List[str]]]) -> List[str]:
    """Normalize a path argument into a flat list of non-empty file path strings."""
    if paths is None:
        return []

    if isinstance(paths, str):
        # UI payloads may arrive as a single path or a semicolon-delimited list.
