...
class AddTableDialog:
    ...
    def _build_basic_tab(self, parent):
        ...
        # Optional explicit numeric format (Python format spec mini-language)
        value_format_frame = ttk.Frame(format_frame)
        value_format_frame.pack(fill=tk.X, pady=5)
        ttk.Label(
            value_format_frame,
            text=self._t("ui.labels.value_format_optional", "Value Format (optional):"),
            width=20
        ).pack(side=tk.LEFT)
        self.value_format_var = tk.StringVar(value="")
        ttk.Entry(value_format_frame, textvariable=self.value_format_var, width=20).pack(side=tk.LEFT, padx=5)
        ttk.Label(
            format_frame,
            text=self._t(
                "ui.messages.value_format_examples",
                "Examples: .4f (fixed), .3e (scientific), .6g (general)"
            ),
            font=("Arial", 8),
            foreground="gray"
    ...
    def _build_table_config(self) -> Dict:
        ...
        value_format = self.value_format_var.get().strip()
        if value_format:
            config['value_format'] = value_format
